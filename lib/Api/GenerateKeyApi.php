...
class GenerateKeyApi
{
     ...
     *
     * @param  \MergeATSClient\Model\GenerateRemoteKeyRequest $generate_remote_key_request generate_remote_key_request (required)
     *
    ...
    {
        list($response) = $this->generateKeyCreateWithHttpInfo($generate_remote_key_request);
        return $response;
    }
     ...
     *
     * @param  \MergeATSClient\Model\GenerateRemoteKeyRequest $generate_remote_key_request (required)
     *
     * @throws \MergeATSClient\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     ...
     */
    public function generateKeyCreateAsync($generate_remote_key_request)
    {
        return $this->generateKeyCreateAsyncWithHttpInfo($generate_remote_key_request)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    public function generateKeyCreateRequest($generate_remote_key_request)
    {
        // verify the required parameter 'generate_remote_key_request' is set
        if ($generate_remote_key_request === null || (is_array($generate_remote_key_request) && count($generate_remote_key_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $generate_remote_key_request when calling generateKeyCreate'
            );
        }

        $resourcePath = '/generate-key';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($generate_remote_key_request)) {
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($generate_remote_key_request));
            } else {
                $httpBody = $generate_remote_key_request;
            }
